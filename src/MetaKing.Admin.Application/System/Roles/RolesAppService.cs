...
namespace MetaKing.Admin.System.Roles
{
    //[Authorize(IdentityPermissions.Roles.Default, Policy = "Admin")]
    [AllowAnonymous]
        ...
        CreateUpdateRoleDto>, IRolesAppService
    {
        ...
        public async override Task<RoleDto> CreateAsync(CreateUpdateRoleDto input)
        {
            ...
            if (isNameExisted)
            {
                throw new BusinessException(MetaKingDomainErrorCodes.RoleNameAlreadyExists)
        ...
        public async Task<GetPermissionListResultDto> GetPermissionsAsync(string providerName, string providerKey)
        {
            ...
            foreach (var group in groups.Where(x => x.Name.StartsWith("AbpIdentity") || x.Name.StartsWith("MetaKingAdmin")))
            {
